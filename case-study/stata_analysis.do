...
logout,save(描述性统计结果n)word replace:tabstat Innov AI_utilize Size Age ROA Leverage Growth Rdintensity,s(N mean sd min max) f(%12.4f) c(s)		 
...
*工具变量 对应表4的（4）、（5）列
*egen yins = group(industry_group year )
...
*gen Absorptive2_AI=Rdintensity*AI_utilize //交互项计算
*gen Absorptive2 = Rdintensity
